...
ulong fd_txn_parse_core(uchar const *payload,
                        ulong payload_sz,
                        void *out_buf,
...
CHECK_LEFT(FD_TXN_ACCT_ADDR_SZ *acct_addr_cnt);
ulong acct_addr_off = i;
i += FD_TXN_ACCT_ADDR_SZ * acct_addr_cnt;
CHECK_LEFT(FD_TXN_BLOCKHASH_SZ);
ulong recent_blockhash_off = i;
i += FD_TXN_BLOCKHASH_SZ;

ushort instr_cnt = (ushort)0;
READ_CHECKED_COMPACT_U16(bytes_consumed, instr_cnt, i);
i += bytes_consumed;
...
for (ulong j = 0UL; j < instr_cnt; j++)
{
  ...
  CHECK_LEFT(MIN_INSTR_SZ);
  uchar program_id = payload[i];
  i++;
  READ_CHECKED_COMPACT_U16(bytes_consumed, acct_cnt, i);
  i += bytes_consumed;
  CHECK_LEFT(acct_cnt);
  ulong acct_off = i;
  for (ulong k = 0; k < acct_cnt; k++)
  {
    max_acct = fd_uchar_max(max_acct, payload[k + i]);
  }
  i += acct_cnt;
  READ_CHECKED_COMPACT_U16(bytes_consumed, data_sz, i);
  i += bytes_consumed;
  CHECK_LEFT(data_sz);
  ulong data_off = i;
  i += data_sz;
...
#define MIN_ADDR_LUT_SIZE (34UL)
  READ_CHECKED_COMPACT_U16(bytes_consumed, addr_table_cnt, i);
  i += bytes_consumed;
  ...
  {
    CHECK_LEFT(FD_TXN_ACCT_ADDR_SZ);
    ulong addr_off = i;
    i += FD_TXN_ACCT_ADDR_SZ;

    ushort writable_cnt = 0;
    ushort readonly_cnt = 0;
    READ_CHECKED_COMPACT_U16(bytes_consumed, writable_cnt, i);
    i += bytes_consumed;
    CHECK_LEFT(writable_cnt);
    ulong writable_off = i;
    i += writable_cnt;
    READ_CHECKED_COMPACT_U16(bytes_consumed, readonly_cnt, i);
    i += bytes_consumed;
    CHECK_LEFT(readonly_cnt);
    ulong readonly_off = i;
    i += readonly_cnt;
...
if (FD_LIKELY(counters_opt))
  counters_opt->success_cnt++;
if (FD_LIKELY(payload_sz_opt))
  *payload_sz_opt = i;
